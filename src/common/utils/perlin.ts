...
export const perlin = {
  ...
  seed: function seed (seed: number) {
    if (seed > 0 && seed < 1) {
      // Scale the seed out
      seed *= 65536;
    }
  ...
  // 2D simplex noise
  simplex2d: function(xin: number, yin: number) {
  ...
  // 3D simplex noise
  simplex3d: function(xin: number, yin: number, zin: number) {
  ...
  // 2D Perlin Noise
  noise2d: function(x: number, y: number) {
  ...
  // 3D Perlin Noise
  noise3d: function(x: number, y: number, z: number) {
    // Find unit grid cell containing point
